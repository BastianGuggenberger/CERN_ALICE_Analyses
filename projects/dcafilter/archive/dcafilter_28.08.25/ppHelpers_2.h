...
class ppHelpers
{
  public:
    ...
    TChain* getChain(std::string fnames)
    {
      ...
      // assign branches
      #include "ppBranchAssignment_2.h"
    ...
    void getHistos(std::vector<TH1D*> &hs1d)
    {
      // 1D ========================================================================
    ...
    void plotHistograms(TCanvas* cv, TString annText, std::vector<TH1D*> &hs1d, std::string pdfname, double IVMmin, double IVMmax)
    {
      TLatex txt;
      ...
      hs1d[0]->GetXaxis()->SetRangeUser(IVMmin,IVMmax);
    ...
    // check event
    bool isGoodEvent_general(ppConfiguration* ppc)
    {
